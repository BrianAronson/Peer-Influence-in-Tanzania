...
#5 - subset to good camps
  ...
  #  Just wave 3
    dataset<-dataset[sumstats2$Wave==3]
    sumstats2<-sumstats2[sumstats2$Wave==3,]
    datasetg <- sienaGroupCreate(dataset)
...
# #dummyize d.hiv
#     d.hiv[[1]][1]
#     for(i in 1:length(d.hiv)){
#       d.hiv[[i]][]<-ifelse(d.hiv[[i]][1]>.1,0,1)
#     }
# 

        # sumstats2$d.hiv[!((paste(sumstats2$Wave,sumstats2$Camp) %in% paste(goodcamps$Wave,goodcamps$Camp)))]
