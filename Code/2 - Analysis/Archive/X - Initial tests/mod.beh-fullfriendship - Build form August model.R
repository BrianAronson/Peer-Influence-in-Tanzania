...
#for each camp not included in base info, run a model of base info + that camp
      # for(q in 1:length(dataset4)){
        # datasetg <- sienaGroupCreate(c(dataset3,dataset4[c(1,9,13)]))
      datasetg <- sienaGroupCreate(dataset4[c(-1,-9,-13)][c(-1,-6)][c(-1,-3)])
      
      temp<-dataset4[c(-1,-9,-13)][c(-1,-6)][c(-1,-3)]
      
      sc<-sumstats3[which((paste(sumstats3$Wave,sumstats3$Camp) %in% paste(base.info$Wave,base.info$Camp))),]

      st<-sumstats3[which(!(paste(sumstats3$Wave,sumstats3$Camp) %in% paste(base.info$Wave,base.info$Camp))),]
      st<-st[c(-1,-9,-13),][c(-1,-6),][c(-1,-3),]
      
      rbind(sc,st)
      round(colMeans(sc)-colMeans(st),2)
      
      # for(q in 1:length(temp)){
      # datasetg <- sienaGroupCreate(dataset4[c(q,q,q,q,q)])

      
      datasetg <- sienaGroupCreate(dataset2[-3])
      
      
      datasetg<- sienaGroupCreate(dataset2[c(1:5,7:9,11:20,22:23)][c(-9,-19)][c(-4,-8,-9)][-4][-4][-7][-4])  
...
#vi - time test
    #manually determine which effects to test      
        ...
        (timetest<-sienaTimeTest(Model,effects = c(47:48)))
