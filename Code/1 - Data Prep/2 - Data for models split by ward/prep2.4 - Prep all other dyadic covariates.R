...
#3.5 - prep all other dyadic covariates
  ...
  #d - create empty edges for non-ties
      #i - create data.table
          dt2<-data.table(ID1=rep(df$pid,each=nrow(df)),ID2=rep(df$pid,nrow(df)),Camp1=rep(df$camp,each=nrow(df)),Camp2=rep(df$camp,nrow(df)),Ward1=rep(df$ward,each=nrow(df)),Ward2=rep(df$ward,nrow(df)))
  ...
  #f - Create separate matrices and arrays for each camp
      #i - remove cross ward ties from data.tables
          tnet1<-tnet1[tnet1$Ward1==tnet1$Ward2,]
          tnet2<-tnet2[tnet2$Ward1==tnet2$Ward2,]
      #i2 - save as temporary object for summary stats
          dyadicv1[[q]]<-tnet1
          dyadicv2[[q]]<-tnet2
          
      #kill bad data
          tnet1<-tnet1[tnet1$Camp1 %in% camps1 & tnet1$Camp2 %in% camps1,]
          tnet2<-tnet2[tnet2$Camp1 %in% camps2 & tnet2$Camp2 %in% camps2,]
          
      
      #ii - split tnetworks into lists by camps
          ntnet1<-split(tnet1,tnet1$Ward1)
          ntnet2<-split(tnet2,tnet2$Ward1)
