    ...
    #for dv versions
        ...
        #iv - convert to siena objects
            ...
            dv.ffriendship<-dv.farrayl
              ...
              #         # #number of clusters per camp
              #         #   sapply(clnum,function(x) length(unique(x)))
              #         # #camp that didn't cluster
              #         #   friendship[sapply(clnum,function(x) length(unique(x)))==1]
              #         # #cluster sizes
              #         #   table(unlist(sapply(clnum,function(x) as.numeric(table(x)))))
              #         # #camp with giant cluster
              #         #   friendship[sapply(clnum,function(x) 54%in%as.numeric(table(x)))]
              #         #   clnum[sapply(clnum,function(x) 54%in%as.numeric(table(x)))]
              #         #   
              #         #   length(unlist(ffipv))
              #         #   length(unlist(clipv))
              #         #   length(unlist(age))
              #         #   
              #         #   
              #         #   (rowSums(clusters[[1]])-1)
              #         #   
              #         # 
              #         #   #
              #                   # cor(c(unlist(clusters[1:59])),c(c(unlist(fnet1[1:59]))))
              #           # cor(c(unlist(tl)),c(c(unlist(clusters[1:59]))))
              #           # cor(c(unlist(tl)),c(c(unlist(fnet1[1:59]))))
              #           # 
              # 
              # #           temp<-dist(fnet1[[1]])
              # # temp[,1]          
