#3 - Prep network data
{
    ...
    #c - have empty edges for non-ties
        #i - Create an empty edge list with all possible (and impossible) ties
            dt<-data.table(ID1=rep(df$pid,each=nrow(df)),ID2=rep(df$pid,nrow(df)),Camp1=rep(df$camp,each=nrow(df)),Camp2=rep(df$camp,nrow(df)))
    ...
    #d - set impossible ties to structural 0s (10)
        ...
        #ii - non responders
            nonresponders<-df2$pid[df2$b_respond==0]
    ...
    #e - Create separate matrices and arrays for each camp
        #i - remove cross camp ties from data.tables
            net1<-net1[net1$Camp1==net1$Camp2,]
            net2<-net2[net2$Camp1==net2$Camp2,]
            net3<-net3[net3$Camp1==net3$Camp2,]
        #ia - for summary stats; save net info as separate name
            sumnet1<-net1
            sumnet2<-net2
            sumnet3<-net3
        #ii - split networks into lists by camps
            nnet1<-split(net1,net1$Camp1)
            nnet2<-split(net2,net2$Camp1)
            nnet3<-split(net3,net3$Camp1)
