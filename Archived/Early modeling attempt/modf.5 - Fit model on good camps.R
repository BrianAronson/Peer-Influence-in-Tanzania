...
#2 - find where rate effects don't work
    ...
    #i.if some places don't converge at all, remove and redo
        ...
        if(any(superhigh)){
            ...
            sumstats3<-sumstats3[!superhigh,]
            source("modf.3 - Create effects for algorithm.R")
            next()
        }
#3 - save useful info
    #save thetas of selection effects
        effects<-!grepl("^constant friendship rate",myeff[myeff$include==T,]$effectName) &!grepl("^unspecified",myeff[myeff$include==T,]$effectName)
        thetas<-Model$theta[effects]
    #save good camp names
        goodcamps<-paste(sumstats3$Wave, sumstats3$Camp)

#4 - Subset camps, but this time by places that weren't tested but might converge
    dataset2<-dataset
    sumstats3<-sumstats2
    removedids<-vector()
    removedids<-c(removedids,which(sumstats3$Camp==29))
    removedids<-c(removedids,which(sumstats3$N<=10))
    removedids<-c(removedids,which(sumstats3$jaccard<=.1))
    ...
    removedids2<-which(goodcamps%in% paste(sumstats3$Wave, sumstats3$Camp))
    keeps<-setdiff(1:length(dataset2),removedids2)
    dataset2[removedids2]<-NULL
    sumstats3<-sumstats3[keeps,]
    datasetg <- sienaGroupCreate(dataset2)
...
#6 - set effects to parameters among good camps
    effects<-!grepl("^constant friendship rate",myeff[myeff$include==T,]$effectName) &!grepl("^unspecified",myeff[myeff$include==T,]$effectName)
    myeff[myeff$include==T,][effects,]$fix<-T
    myeff[myeff$include==T,][effects,]$initialValue<-thetas

#7 - run model and see which work
    Model <- siena07( myalgorithm, data = datasetg, effects = myeff, nbrNodes=25, useCluster=T, initC=T, returnDeps=TRUE,batch=T, verbose = F)
    SaveResults()
